...
# 
# # new_ids <- tibble(subj_code = unique(df$subj_code),
# #                   sub_ID = 5000:(4999 + length(unique(df$subj_code)))) # after review unique identifier
# 
# # After review 2: final batch
# 
# new_ids <- tibble(subj_code = unique(df$subj_code),
#                   sub_ID = 7000:(6999 + length(unique(df$subj_code)))) # after review unique identifier
# 
# # Override old IDs:
# 
...
# 
# write_csv(df,
#           '../ratings/combined_byWord_and_subject_anonymized_after_review_final.csv')
# # write_csv(df,
# #           '../ratings/combined_byWord_and_subject_anonymized_after_review.csv')
# # write_csv(df,
# #           '../ratings/combined_byWord_and_subject_anonymized.csv')
```

**Update 09/11/2022**: Load new data after review:

```{r}
df_new <- read_csv('../ratings/combined_byWord_and_subject_anonymized_after_review.csv') %>% 
  mutate(subj_code = as.character(subj_code))
```

Load final batch of missing words:

```{r}
df_final <- read_csv('../ratings/combined_byWord_and_subject_anonymized_after_review_final.csv') %>% 
  mutate(subj_code = as.character(subj_code))
```
...
```{r}
df <- bind_rows(df, df_new, df_final)
```
...
1271 participants before review. 1402 after review. 1416 final batch.

## Check reaction time (RT) distribution
...
4 data points for which there is no `rt` and `key` data.
...
There definitely seems to be a few free riders here. They basically just clicked themselves through the experiment, except for the beginning and end.
...
Even 500ms is just about 2% of the data. This will be our threshold for exclusion as it is next to nigh impossible to make a semantically deep judgment of something as involved as iconicity (requiring comparison of phonological form and semantics):
...
That's 2% of the data. Previously we used 15s as a sensible cut-off value, but the reviewers flagged this and we concur that there is no intrinsic reason to prefer long RTs, which may involve people leaving their desk or opening up another tab and coming back to the experiment:

```{r}
# Exclusion 3... based on rt > 15s

# df <- filter(df, rt <= 15000)
```
...
2.2% data loss, a total of 3885 data points.
...
Average RT and SD across subjects:
...
Subject "468" for example had 137 times key = 7. That's a bit suspicious. Let's merge this with the per-subject-N to see the proportion of responses that are the same.
...
16 out of 1402 participants will be excluded, that's 1%.
...
That's 70 participants, 5% of the total number of participants.
...
7,705 data points, 4.4% of the data. After final batch: 7,858, still 4.4%.
...
How many are known by 70% of all participants?
...
```{r}
nrow(filter(icon, n < 10))
```

Check whether those that we would exclude based on this criterion have a lower proportion of being known:

```{r}
filter(icon, n < 10) %>% 
  summarize(min_known = min(prop_known),
            max_known = max(prop_known),
            mean_known = mean(prop_known))
```

Check the distribution of this:
...
It's bimodal. Let's get rid of those for which proportion known is less than 80%.
...
Check for how many we now have less than 10 ratings:

```{r}
filter(icon, n < 10) %>% nrow()
```

Previously: 285 words. (Now 95 words) (**Update 09/11/2022**: Get those 285 words of here to get Gary to collect new data for those words:)

```{r}
# write_csv(filter(icon, n < 10), '../ratings/final_bout.csv')
```

**Update 09/11/2022**What's going on with the ones for which we have less than 10 now? What sorts of words are they?

```{r}
filter(icon, n < 10)
```

Get those for which we have at least 10 ratings:

```{r}
icon <- filter(icon, n >= 10)
```
...
12.0% in tota, 21,715 raw trials.
...
```{r}
write_csv(filter(df, word %in% final_words, !is.na(key)),
